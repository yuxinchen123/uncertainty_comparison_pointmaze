...
ROUND-2 RE-RUN of the final deliverable, on the improved trainer (compiled post-rollout body,
per-step work hoisted out of the rollout loop, buffer writes folded into the compiled step —
about 1.8x faster per iteration than the round-1 code the first campaign used). Same
configuration otherwise, so the two campaigns are directly comparable.
...
| execution | one-graph CUDA-graph capture (rollout+post+update), TF32, compiled post body, rollout hoist, folded buffer writes, fused capturable Adam |
...
Commit `d848699` (branch Use-RLexplore-RND), working tree clean for 09_parallelization at
