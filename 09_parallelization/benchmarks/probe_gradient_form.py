...
def build(n_copies, gradient_buffer, layout="copy_major"):
...
def main():
    ...
    ap.add_argument("--parameter-layout", default="copy_major",
                    choices=["copy_major", "parameter_major"])
    ap.add_argument("--tag", default="")
    args = ap.parse_args()
    C = args.n_copies
    # the parameter-major layout gives no copy of a per-copy row for one program to walk, so the
    # buffer form does not exist there and only the no-buffer programs are measured
    with_buffer = args.parameter_layout == "copy_major"
    ...
    print(f"\n== the programs the gradient forms differ in, {C} copies, "
          f"{args.parameter_layout} ==")
    per_step = {}
    if with_buffer:
        buf, raw_buf, flat_grad = build(C, True, args.parameter_layout)
        one_buffer = F32 * buf._flat.numel()
        ...
        per_step["buffer form"] = sum(r["microseconds"] for r in rows[:3])
        del buf, raw_buf, flat_grad
        torch.cuda.empty_cache()

    direct, raw_direct, grads = build(C, False, args.parameter_layout)
    ...
    per_step["no-buffer form"] = sum(r["microseconds"] for r in rows[before:])
    ...
    p.write_text(json.dumps({
        "n_copies": C, "parameter_layout": args.parameter_layout,
