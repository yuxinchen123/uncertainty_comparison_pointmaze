...
# at the small ones the earlier rounds were tuned on, the profiles repeated on the changed
...
# the small copy counts keep the gradient buffer, because the twenty-one-program optimizer costs
# more there than the copy it removes. Their remaining choice is whether the gradient limit rides
# along with that copy, which has only been measured at 1,024 and 4,096
run n0_fused_limit_small "$W/benchmarks" "\
  $PYT bench_torch_change.py --knob set \
       --off 'gradient_buffer=True,parameter_layout=copy_major' \
       --on 'gradient_buffer=True,parameter_layout=copy_major,fuse_copy_and_limit=True' \
       --copies 8 32 128 512 --rounds 11 --iters 20 --style epoch_minibatch --tag _small"

# every gate, now against the configuration that ships
run n1_gates "$W/ppo/torch_ppo" "\
...
run n2_after_curves "$W/benchmarks" "\
...
run n3_small_sizes "$W/benchmarks" "\
...
run n4_after_profiles "$W/benchmarks" "\
...
run n6_equivalence "$W/benchmarks" "\
  $PYT compare_revisions.py --a $R5 --b '' --n-copies 32 --iters 1 --tag _r6"

# the compiler-generated multiplication once more, now that every arm primes its statistics from
# the same seed, so the accuracy columns compare kernels rather than different batches
run n7_epilogue_seeded "$W/benchmarks" "\
