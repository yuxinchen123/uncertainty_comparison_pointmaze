#!/usr/bin/env python3
"""
Scalar Ensemble Noise-Based RND-Linear (LS) Method
...
from scalar_ensemble_methods import ScalarEnsembleRNDLinearLSMethod
...
def parse_args():
    parser = argparse.ArgumentParser(description="Scalar Ensemble Noise-Based RND-Linear (LS) Testing")
    ...
    parser.add_argument("--project_name", type=str, default="scalar-ensemble-rnd-linear-ls", help="WandB project name")
...
def run_single_experiment(args, device, run_idx):
    ...
    # Create scalar ensemble RND-Linear (LS) method
    ...
    # Train ensemble
    print(f"Training Scalar Ensemble RND-Linear (LS) with K={args.K} predictors...")
    ...
    uncertainty_matrix = normalize_uncertainty_matrix(uncertainty_matrix, maze_map)
    
    # Normalize ground truth for comparison
    ground_truth_normalized = normalize_uncertainty_matrix(ground_truth, maze_map)
    
    # Calculate metrics (use maze_map)
    maze_map_array = np.array(maze_map)
    l2_distance = compute_l2_distance(ground_truth_normalized, uncertainty_matrix, maze_map_array)
    min_c_l1_norm_diff = compute_min_c_l1_norm_diff(ground_truth_normalized, uncertainty_matrix, maze_map_array)
    min_c_l2_norm_diff = compute_min_c_l2_norm_diff(ground_truth_normalized, uncertainty_matrix, maze_map_array)
    min_c_l1_norm_inv = compute_min_c_l1_norm_inv(ground_truth_normalized, uncertainty_matrix, maze_map_array)
    min_c_l2_norm_inv = compute_min_c_l2_norm_inv(ground_truth_normalized, uncertainty_matrix, maze_map_array)
...
def main():
    ...
    if wandb.run is not None:
        ...
        if args.wandb_switch.lower() == "true":
            wandb.init(
                ...
                config=vars(args),
                name=f"scalar_ensemble_rnd_linear_ls_K{args.K}_noise{args.noise_sigma}"
            )
    ...
    print(f"Starting Scalar Ensemble Noise-Based RND-Linear (LS) testing")
    ...
    save_heatmap_to_wandb(
        ...
        title=f"Scalar Ensemble RND-Linear (LS) (K={args.K}, noise_sigma={args.noise_sigma}) - Final Run",
        wandb_switch=(args.wandb_switch.lower() == "true")
    )
