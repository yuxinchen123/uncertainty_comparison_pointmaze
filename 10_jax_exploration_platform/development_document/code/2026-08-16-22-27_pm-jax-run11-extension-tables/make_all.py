...
`platform_development_document.tex`, between the AUTO-GENERATED TABLE markers:
  ...
  - pm-jax-run11-results : one row per algorithm arm at its best configuration in the ORIGINAL
                           sweep, then a double rule, then the same four arms as re-run by the
                           EXTENSION at 1,024 copies for 1,000,038,400 environment steps per copy

This is the extension's generator. The pre-extension one,
`2026-08-16-21-00_pm-jax-run11-tables/`, is left in place as the record of what the table said
before the extension ran; it is not called any more.

Every number is IMPORTED from the run it belongs to — the original sweep's rows from the parent
run's own `code/aggregate.py`, the extension's rows from the extension run's — so the writeup
cannot drift from what each run's own monitor reports. Nothing here loads a shard, scores a copy,
or ranks a configuration. The two modules have the same file name, so they are loaded by path
...
underlined, higher is better for every column that carries numbers, the N column never marked.
It is computed WITHIN each block: the two blocks are different step budgets and different copy
counts, so marking one against the other would compare measurements that are not comparable.
...
def load_run_module(run_dir: Path, module_name: str):
    ...
    The two runs' modules are both called `aggregate`, so a plain `import aggregate` would return
    whichever was imported first and silently score one run with the other's module.

    before: run_dir = <parent run>, module_name = "aggregate_original"
    after:  sys.modules["aggregate_original"] is that run's module, with its own RUN_DIR
    """
...
def block_lines(module, run_dir: Path) -> list:
    """One run's four rows, sorted by whole-run reward and marked within themselves."""
...
def build_results_table() -> str:
    """The tabular block: the original sweep's rows, a double rule, then the extension's rows."""
    original = load_run_module(ORIGINAL_RUN, "aggregate_original")
    extension = load_run_module(EXTENSION_RUN, "aggregate_extension")
    lines = [
    ...
    # the deliberate double rule: everything below it is the extension's longer, wider re-run of
    # the same four configurations, and the caption says so
    lines += ["\\midrule\\midrule"]
    lines += block_lines(extension, EXTENSION_RUN)
