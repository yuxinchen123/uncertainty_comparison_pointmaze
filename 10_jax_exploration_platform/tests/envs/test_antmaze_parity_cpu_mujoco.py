...
Three gates (spec.md, "Correctness gates" 1 and 2):
  1. one-step parity: from 50 states along a C-MuJoCo trajectory, one env step in MJX (float32)
     against one env step in C MuJoCo (float64) — position error bounded by float32 roundoff
     through the solver, not by trajectory divergence.
...
def c_trajectory(env, steps, key):
    ...
    before: nothing; after: qpos [steps+1, nq], qvel [steps+1, nv], acts [steps, nu] (numpy)
    """
    ...
    d.qvel[:] = 0.0
    qs, vs, acts = [d.qpos.copy()], [d.qvel.copy()], []
    for k in range(steps):
        ...
        acts.append(a)
    return np.stack(qs), np.stack(vs), np.stack(acts), m


def test_one_step_parity():
    """From states along a C trajectory, MJX's next state matches C MuJoCo's closely."""
    env = JaxAntMaze(CFG, 1, 1)
    qs, vs, acts, m = c_trajectory(env, 50, jax.random.PRNGKey(11))
    d = mujoco.MjData(m)
    ...
    for k in range(50):
        ...
        worst = max(worst, err)
    print(f"one-step parity: worst |dqpos| over 50 states = {worst:.2e}")
...
def test_short_horizon_trajectory():
    ...
    env = JaxAntMaze(CFG, 1, 1)
    qs, vs, acts, m = c_trajectory(env, 20, jax.random.PRNGKey(5))
    s = env.reset()
    step = jax.jit(env.step)
    errs = []
    for k in range(20):
