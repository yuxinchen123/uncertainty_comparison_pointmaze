"""Benchmark the MJX AntMaze environment step on the graphics card. Writes one JSON beside
this file into results/ and prints one table row per (map, copies) setting.

The measured unit is one ENVIRONMENT step — frame_skip (5) physics steps — under random
torques, jitted alone with a donated state, so the number is the environment's and contains
no agent arithmetic. Copy counts are walked in ascending order so the peak-memory column is
per-row (the JAX high-water mark never resets within a process).

Usage (serval05, while this session holds the H100 lock):
  PYTHONNOUSERSITE=1 python bench_antmaze_env.py --maps umaze medium large \
      --copies 512 1024 2048 4096 --out results/<name>.json
"""
import argparse
import json
import sys
...
def bench(map_name, n_copies, rounds=11, steps_per_round=10):
    ...
    from exploration_platform.envs.antmaze.mjx_antmaze import JaxAntMaze

    env = JaxAntMaze(preset(map_name), n_copies, 1)

    def block(state, key):
        """steps_per_round env steps of keyed random torques, one compiled program."""
        def body(carry, k):
            st = carry
            act = jax.random.uniform(jax.random.fold_in(key, k),
                                     (n_copies, 1, env.act_dim), jnp.float32, -1.0, 1.0)
    ...
    assert bool(jnp.isfinite(state.data.qpos).all()), "state went non-finite during the bench"

    sec = float(np.median(times))
    return {"map": map_name, "copies": n_copies, "n_envs": 1,
...
def main():
    ...
    ap.add_argument("--maps", nargs="+", default=["umaze", "medium", "large"])
    ...
    for map_name in args.maps:
        for c in sorted(args.copies):
            r = bench(map_name, c)
            rows.append(r)
            print(f"{map_name:7s} C={c:5d}: {r['seconds_per_env_step']*1e3:8.3f} ms/env-step  "
                  ...
                  flush=True)
    out = Path(args.out) if args.out else RESULTS / f"antmaze_env_{int(time.time())}.json"
