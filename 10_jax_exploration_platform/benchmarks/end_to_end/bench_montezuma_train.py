"""Benchmark the whole fused training iteration on Montezuma's Revenge — environment, agent and bonus in
...
Usage (serval05, while this session holds the H100 lock):
  PYTHONNOUSERSITE=1 python bench_montezuma_train.py --bonuses rnd_next_state none \
...
def bench(bonus, n_copies, num_steps, n_envs, update_style, rounds=11, iters=10):
    ...
    tr = Runner(cfg, bonus=bonus, env_cfg=MontezumaConfig())
    ...
    return {"map": "montezuma", "bonus": bonus, "copies": n_copies, "n_envs": n_envs,
...
def main():
    ...
    if True:
        for bonus in args.bonuses:
            for c in sorted(args.copies):
                r = bench(bonus, c, args.num_steps, args.n_envs, args.style)
                rows.append(r)
                print(f"montezuma {bonus:15s} C={c:5d}: "
    ...
    out = Path(args.out) if args.out else RESULTS / f"montezuma_train_{int(time.time())}.json"
