...
"best configuration" the parent's script had to choose is here the unit's only cell, and it is
still read through `code/aggregate.best_per_arm` rather than hardcoded, so the figure and the
development document's results table cannot name different configurations. The curve is the mean
episode return over that configuration's 8,192 copies at each recorded window, and the band is plus
or minus one standard error over those copies.

The figure keeps the parent's file name, `pm_jax_run11_reward_curves.pdf`, because it fills the
same slot in the development document (\\Cref{fig:pm-jax-run11-curves}) — but it is written into
THIS run folder, so the parent's own plot survives untouched as the record of the shorter budget.
The document's figure now shows the 100-million-step curves only: two budgets of one arm drawn in
one panel read as two algorithms.
...
the same pair the parent run gave it.
...
def main() -> None:
    ...
    for bonus in ARM_ORDER:
        ...
        # before: env_steps_per_copy in units of steps (up to 100,044,800); after: millions, which
        ...
        axes.plot(steps, mean, color=colour, linestyle=style, linewidth=1.4,
    ...
    axes.set_title("PointMaze large, no reset noise — each arm's best configuration, 100M steps")
