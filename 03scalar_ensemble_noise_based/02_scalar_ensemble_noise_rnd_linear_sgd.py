...
def run_single_experiment(args, device, run_idx):
    ...
    # Train ensemble
    print(f"Training Scalar Ensemble RND-Linear (SGD) with {args.num_heads} predictors...")
...
def main():
    ...
    if wandb.run is not None:
        ...
        if args.wandb_switch.lower() == "true":
            wandb.init(
                ...
                config=vars(args),
                name=f"scalar_ensemble_rnd_linear_sgd_num_heads{args.num_heads}_noise{args.noise_sigma}"
            )
    ...
    save_heatmap_to_wandb(
        ...
        title=f"Scalar Ensemble RND-Linear (SGD) (num_heads={args.num_heads}, noise_sigma={args.noise_sigma}) - Final Run",
        wandb_switch=(args.wandb_switch.lower() == "true")
    )
