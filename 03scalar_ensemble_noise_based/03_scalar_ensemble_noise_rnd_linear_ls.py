...
def run_single_experiment(args, device, run_idx):
    ...
    # Train ensemble
    print(f"Training Scalar Ensemble RND-Linear (LS) with {args.num_heads} predictors...")
...
def main():
    ...
    if wandb.run is not None:
        ...
        if args.wandb_switch.lower() == "true":
            wandb.init(
                ...
                config=vars(args),
                name=f"scalar_ensemble_rnd_linear_ls_num_heads{args.num_heads}_noise{args.noise_sigma}"
            )
    ...
    # Log method heatmaps (both error-based and prediction-based)
    save_heatmap_to_wandb(
        final_result['uncertainty_matrix_errors'],
        title=f"Scalar Ensemble RND-Linear (LS) Errors (num_heads={args.num_heads}, noise_sigma={args.noise_sigma}) - Final Run",
        wandb_switch=(args.wandb_switch.lower() == "true")
    )
    save_heatmap_to_wandb(
        final_result['uncertainty_matrix_predictions'],
        title=f"Scalar Ensemble RND-Linear (LS) Predictions (num_heads={args.num_heads}, noise_sigma={args.noise_sigma}) - Final Run",
        wandb_switch=(args.wandb_switch.lower() == "true")
    )
