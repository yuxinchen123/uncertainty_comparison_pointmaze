#!/usr/bin/env python3
"""
Ensemble RND-Linear (SGD) Method Testing

Focused testing of Ensemble RND-Linear with SGD training.
K=10 linear predictors, each trained with SGD on bootstrap samples.
"""
...
from ensemble_uncertainty_methods import EnsembleRNDLinearSGDMethod
...
def parse_args():
    parser = argparse.ArgumentParser(description="Ensemble RND-Linear (SGD) Testing")
    ...
    parser.add_argument("--project_name", type=str, default="ensemble-rnd-linear-sgd", help="WandB project name")
...
def run_single_experiment(args, device, run_idx):
    ...
    # Create ensemble RND-Linear (SGD) method
    ...
    # Train ensemble
    print(f"Training Ensemble RND-Linear (SGD) with {args.num_heads} predictors...")
...
def main():
    ...
    if args.wandb_switch.lower() == "true":
        wandb.init(
            ...
            config=vars(args),
            name=f"ensemble_rnd_linear_sgd_num_heads{args.num_heads}_samples{args.num_samples}"
        )
    
    print(f"Starting Ensemble RND-Linear (SGD) testing")
    ...
    save_heatmap_to_wandb(
        ...
        title=f"Ensemble RND-Linear (SGD) (num_heads={args.num_heads}) - Final Run",
        wandb_switch=(args.wandb_switch.lower() == "true")
    )
