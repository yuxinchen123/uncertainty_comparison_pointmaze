...
def run_single_experiment(args, device, run_idx):
    ...
    # Train ensemble
    print(f"Training Ensemble RND with {args.num_heads} predictors...")
    ...
    l2_distance = compute_l2_distance(ground_truth_normalized, uncertainty_matrix_normalized, maze_map)
    # The 4 new metrics: use original (unnormalized) GT and predictions
    min_c_l1_norm_diff = compute_min_c_l1_norm_diff(ground_truth, uncertainty_matrix_original, maze_map)
    min_c_l2_norm_diff = compute_min_c_l2_norm_diff(ground_truth, uncertainty_matrix_original, maze_map)
    min_c_l1_norm_inv = compute_min_c_l1_norm_inv(ground_truth, uncertainty_matrix_original, maze_map)
    min_c_l2_norm_inv = compute_min_c_l2_norm_inv(ground_truth, uncertainty_matrix_original, maze_map)
...
def main():
    ...
    if args.wandb_switch.lower() == "true":
        wandb.init(
            ...
            config=vars(args),
                name=f"ensemble_rnd_num_heads{args.num_heads}_samples{args.num_samples}"
        )
    ...
    save_heatmap_to_wandb(
        ...
        title=f"Ensemble RND (num_heads={args.num_heads}) - Final Run",
        wandb_switch=(args.wandb_switch.lower() == "true")
    )
