    ...
    def _log_heatmap(self, visit_counts: np.ndarray, prefix: str, step: int, goal_idx: Optional[int] = None):
        ...
        try:
            ...
            if len(visit_counts.shape) == 3:
                ...
                goal_cells = self.goal_cells if self.goal_cells else [None] * visit_counts.shape[0]
                figures = create_multi_goal_heatmap(
                    visit_counts,
                    maze_map=self.maze_map,
                    goal_cells=goal_cells,
                    title_prefix=f"{prefix.capitalize()} Visit Count",
                )
                
                # Log each goal's heatmap
                for goal_idx, fig in enumerate(figures):
                    wandb.log({
                        f"{prefix}/visit_count_heatmap/goal_{goal_idx + 1}": wandb.Image(fig)
                    }, step=step)
            ...
            # Handle single-goal mode (2D array)
                ...
                goal_cell = None
                if self.goal_cells and len(self.goal_cells) > 0:
                ...
                fig = create_visit_count_heatmap(
                    visit_counts,
                    maze_map=self.maze_map,
                    title=f"{prefix.capitalize()} Visit Count Heatmap (Step {step})",
                    goal_cell=goal_cell,
                    start_cell=self.start_cell,
                )
                
                wandb.log({
                    f"{prefix}/visit_count_heatmap": wandb.Image(fig)
                }, step=step)
                import matplotlib.pyplot as plt
                plt.close(fig)
