...
class Method:
    ...
    name = "deep_lastlayer_cfn_capped_wide"
    ...
    def __init__(self, seed: int, obs_dim=4):
        ...
        self.d = d
        self.feat_dim = 1024 if self.is_image else 256
        self.trunk = make_trunk(obs_dim, self.feat_dim, seed, "feat")
        ...
        self.Lam = torch.zeros(self.feat_dim, self.feat_dim, dtype=torch.float64, device=self.device)
        self.Bmat = torch.zeros(self.feat_dim, d, dtype=torch.float64, device=self.device)
