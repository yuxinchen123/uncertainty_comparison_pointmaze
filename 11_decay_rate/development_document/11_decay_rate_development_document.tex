...
\midrule
$x_i$ & evaluation state $i$ (maze input $[x, y, 0, 0]$, AntMaze observation, or Atari frame; \Cref{sec:platform} lists the sets) \\
...
044 & gradient conv CFN Adam 1e-4, atari (discarded) & 1.654 & 1.343 & 0.00 & -0.699 & 0.368 \\
045 & gradient conv CFN Adam 1e-3, atari (discarded) & 1.155 & 0.994 & 0.00 & -0.807 & 0.314 \\
046 & gradient conv CFN AdaGrad 1e-2, atari (discarded) & 1.779 & 1.384 & 0.00 & -0.469 & 0.210 \\
047 & gradient conv CFN SGD-1/t 1e-1, atari (discarded) & 2.678 & 2.466 & 0.00 & -2.800 & 0.136 \\
052 & replay-buffer CFN (4 Adam steps/visit), maze+antmaze & 0.863 & 0.675 & 0.00 & -0.909 & 0.198 \\
050 & deep shrink head, atari 512 frames vs 256 features & 1.814 & 1.534 & 0.00 & -2.351 & 0.283 \\
051 & deep coin-flip head, atari & 1.871 & 1.534 & 0.00 & -0.961 & 0.078 \\
053 & deep shrink, HELDOUT vectors & 1.935 & 0.292 & 0.00 & -0.944 & 1.282 \\
054 & deep coin-flip, HELDOUT vectors & 1.678 & 0.264 & 0.00 & -0.609 & 0.549 \\
055 & deep coin-flip, NONUNIFORM vectors & 0.618 & 0.138 & 0.00 & -0.692 & 0.198 \\
056 & deep shrink, NONUNIFORM vectors & 1.216 & 0.711 & 0.00 & -0.352 & 0.145 \\
057 & deep shrink, HELDOUT atari & 1.652 & 1.202 & 0.00 & -2.293 & 1.048 \\
058 & deep coin-flip, HELDOUT atari & 1.771 & 1.202 & 0.00 & -0.900 & 0.168 \\
059 & deep shrink head, 1024 image features, atari & 0.038 & 0.038 & 0.00 & -0.500 & 0.000 \\
060 & deep coin-flip head, 1024 image features, atari & 0.578 & 0.064 & 0.00 & -0.504 & 0.006 \\
val 201 & deep shrink, vectors uniform, 30 seeds & 0.038 & 0.038 & 0.00 & -0.500 & 0.000 \\
val 202 & deep coin-flip, vectors uniform, 30 seeds & 0.055 & 0.030 & 0.00 & -0.502 & 0.003 \\
val 203 & deep coin-flip nonuniform 30 seeds WITH THE ABSOLUTE 1e-8 \,\ldots (discarded) & 1.675 & 0.702 & 0.00 & -1.125 & 0.495 \\
val 206 & capped coin-flip head, NONUNIFORM vectors, 30 seeds & 0.405 & 0.143 & 0.00 & -0.651 & 0.131 \\
val 207 & capped coin-flip head, 1024 image features, atari uniform & 0.578 & 0.064 & 0.00 & -0.504 & 0.006 \\
val 208 & capped+wide coin-flip head, atari NONUNIFORM & 0.734 & 0.068 & 0.00 & -0.865 & 0.219 \\
val 209 & capped coin-flip head, HELDOUT vectors, 30 seeds & 1.866 & 0.279 & 0.00 & -0.680 & 0.528 \\
val 210 & capped+wide coin-flip head, HELDOUT atari & 3.006 & 0.191 & 0.00 & -0.678 & 0.412 \\
\bottomrule
\end{longtable}}
...
\begin{figure}[H]
\centering
\includegraphics[width=\linewidth]{figures/neural_curves.pdf}
\caption{The phase-2 story in per-state curve fans (seed-mean; one thin line per state; grey
line $= \min(1, n^{-1/2})$). Left: the best pure-gradient coin-flip network on the cell
midpoints --- gradient steps lag the running coin mean and the fan never settles on the
oracle. Middle: the exact coin-flip head on Atari with $256$ features --- every curve floors
at the target component outside the feature span. Right: the same head with $1024$ features
--- the floor is gone and the fan sits on the oracle up to the statistical fluctuation.}
\label{fig:neural-curves}
\end{figure}
...
replay-buffer coin-flip net (4 Adam steps/visit) & 0.863 & 0.675 & -0.909 & 0.198 \\
...
\midrule
\multicolumn{5}{@{}l}{\textbf{Atari frames (uniform)} (10 seeds)} \\
conv trunk (1024 feat.) + exact shrink head & \textbf{0.038} & \textbf{0.038} & -0.500 & 0.000 \\
conv trunk (1024 feat.) + exact coin-flip head & \underline{0.578} & \underline{0.064} & -0.504 & 0.006 \\
conv trunk (1024 feat.) + capped coin-flip head & 0.578 & 0.064 & -0.504 & 0.006 \\
gradient coin-flip conv net, Adam $10^{-3}$ & 1.155 & 0.994 & -0.807 & 0.314 \\
gradient coin-flip conv net, Adam $10^{-4}$ & 1.654 & 1.343 & -0.699 & 0.368 \\
gradient coin-flip conv net, AdaGrad $10^{-2}$ & 1.779 & 1.384 & -0.469 & 0.210 \\
conv trunk (256 feat.) + exact shrink head & 1.814 & 1.534 & -2.351 & 0.283 \\
conv trunk (256 feat.) + exact coin-flip head & 1.871 & 1.534 & -0.961 & 0.078 \\
\midrule
\multicolumn{5}{@{}l}{\textbf{held-out 20\% (vectors)} (10 seeds)} \\
deep trunk + exact coin-flip head & \textbf{1.678} & \textbf{0.264} & -0.609 & 0.549 \\
deep trunk + capped coin-flip head (30 seeds) & \underline{1.866} & \underline{0.279} & -0.680 & 0.528 \\
deep trunk + exact shrink head & 1.935 & 0.292 & -0.944 & 1.282 \\
\midrule
\multicolumn{5}{@{}l}{\textbf{held-out 20\% (Atari)} (10 seeds)} \\
conv trunk + exact shrink head & \textbf{1.652} & \underline{1.202} & -2.293 & 1.048 \\
conv trunk + exact coin-flip head & \underline{1.771} & 1.202 & -0.900 & 0.168 \\
conv trunk (1024 feat.) + capped coin-flip head & 3.006 & \textbf{0.191} & -0.678 & 0.412 \\
\midrule
\multicolumn{5}{@{}l}{\textbf{nonuniform visitation (vectors)} (10 seeds)} \\
deep trunk + capped coin-flip head (30 seeds) & \textbf{0.405} & \underline{0.143} & -0.651 & 0.131 \\
deep trunk + exact coin-flip head & \underline{0.618} & \textbf{0.138} & -0.692 & 0.198 \\
deep trunk + exact shrink head & 1.216 & 0.711 & -0.352 & 0.145 \\
\midrule
\multicolumn{5}{@{}l}{\textbf{nonuniform visitation (Atari)} (10 seeds)} \\
conv trunk (1024 feat.) + capped coin-flip head & \textbf{0.734} & \textbf{0.068} & -0.865 & 0.219 \\
\bottomrule
\end{tabular}
...
\subsection{What the generalization domains and regimes measure}
\label{sec:neural-generalization}

Four findings, all visible in \Cref{tab:neural}:
\begin{enumerate}[label=(\roman*),leftmargin=*]
\item \textbf{The exact heads transfer to real high-dimensional states.} On the pooled maze
  plus AntMaze-observation battery the shrink head reproduces its phase-1 numbers to the
  digit ($0.0381$, slope $-0.500000$, zero spread --- \runref{exp 048}) and the coin-flip
  head sits at its statistical floor ($0.054$ --- \runref{exp 049}); under non-uniform
  visitation the coin-flip head tracks each real AntMaze state's own count
  (\runref{exp 055}, $0.62$ worst / $0.14$ mean) while the shrink head fails exactly as its
  phase-1 sibling did (\runref{exp 056}).
\item \textbf{Feature width is the capacity law.} On Atari --- $512$ distinct frames against
  $256$ trunk features --- both heads hit a hard floor near $0.25$--$0.29$
  (\runref{exp 050}, \runref{exp 051}): the component of the target outside the feature span
  cannot be removed by any last-layer update, and the early slope fits ($-2.35$) are
  artifacts of fitting a knee-then-floor curve. This is the same law the phase-1 dictionary
  obeyed (insertion capacity $\ge$ distinct states): \emph{the feature dimension bounds how
  many states can be counted individually}. The converse holds too: widening the image
  trunk to $1024$ features restores machine-exactness for the shrink head on all $512$
  frames ($0.0381$, slope $-0.500000$, zero spread --- \runref{exp 059}) and removes the
  coin-flip head's floor (dev\_mean $0.064$, at its statistical floor --- \runref{exp 060};
  its worst frames are near-duplicates whose nearly identical features share one count, the
  image-domain face of the pseudo-count dial below).
\item \textbf{Held-out states expose the pseudo-count trade-off.} In the held-out regime the
  trained states stay on their count curves (dev\_mean $0.26$--$0.29$ on the vector battery)
  but never-visited states are dragged partway down by the trunk's generalization
  (\runref{exp 053}, \runref{exp 054}): under the strict-count reading that is leakage and
  the metric punishes it; under the pseudo-count reading, a state that resembles trained
  ones \emph{should} inherit part of their familiarity. The metric takes the strict view; a
  deployment choosing bandwidth or trunk width is choosing a point on this dial, and the
  off-support fields of \Cref{fig:offsupport} are the same trade-off seen from the other
  side. The Atari held-out cell makes the point sharply (\runref{val 210}): the widened
  trunk keeps every trained frame on its count curve (dev\_mean $0.19$ against $1.20$ at
  $256$ features), and the single worst state of the whole matrix is a held-out frame
  nearly identical to a trained one, which inherits the trained frame's full decay --- the
  strict-count metric maxing out on exactly the state the pseudo-count reading calls
  familiar.
\item \textbf{Two deployment rules the counting head needs.} A validation rerun on a
  different node exposed both. First, the head's ridge must scale with the accumulated
  feature energy: with an absolute $10^{-8}$ ridge the early, rank-deficient normal
  equations amplify null-space noise by its reciprocal, and which rounding noise lands there
  is decided by the node's linear-algebra kernels --- the same seed read bonus values near
  $30$ at early checkpoints on one node and none on another (\runref{val 203}, kept as the
  record). Second, the readout is capped at $1$, exactly as the count oracle
  $\min(1, m^{-1/2})$ is capped: a state cannot read as more novel than never-visited, and
  the cap removes the legitimate-but-large least-squares coefficients along barely-visited
  directions from the readout (\runref{exp 061}; both fixes together: maximum bonus exactly
  $1$, late accuracy unchanged).
\item \textbf{Replay narrows but does not close the gradient gap.} The replay-buffer
  coin-flip network (\runref{exp 052}) reaches $0.86$ worst / $0.67$ mean --- far better
  than any per-visit gradient variant, an order short of the exact heads, and carrying its
  own forgetting horizon (the ring bounds the count any state can accumulate).
\end{enumerate}
...
\item \textbf{The neural forms transfer, and the counting belongs in the last layer.}
  Keeping the deep network for representation and making the last layer exact carries every
  phase-1 property onto real 29-dimensional AntMaze observations unchanged (shrink head:
  $0.0381$, slope $-0.500000$, zero spread; coin-flip head: the statistical floor, plus
  per-state count tracking under non-uniform visitation). Pure gradient training of the same
  constructions — the published coin-flip network's regime — cannot track the running coin
  mean at one step per visit on any domain, and a replay buffer narrows the gap by an order
  without closing it.
\item \textbf{Feature width is the capacity law of neural counting.} With more distinct
  states than feature dimensions ($512$ Atari frames against $256$ features) every
  last-layer method floors at the target's unrepresentable component; the phase-1 dictionary
  obeyed the same law through its insertion capacity. Counting $P$ states individually needs
  a feature dimension of at least $P$ — width is not a tuning knob here but the resource that
  buys per-state resolution.
\item \textbf{The deliverable, stated as an algorithm.} Take an RND pipeline; keep its deep
  trunk (MLP or conv); freeze the trunk (random biases $N(0, 0.5^2)$ for inputs of dimension
  $\le 8$); zero-initialize the last layer and normalize the frozen reference per input so
  the bonus is exactly $1$ at every never-visited state; then update ONLY the last layer with
  one of the two exact heads — the residual-encoded shrink map for uniform-visitation
  exactness, or the coin-flip normal equations for per-state counts under arbitrary
  visitation — with feature width at least the number of states to be resolved. Everything
  else in this document measures what is lost when any one of those ingredients is replaced
  by ordinary gradient descent.
