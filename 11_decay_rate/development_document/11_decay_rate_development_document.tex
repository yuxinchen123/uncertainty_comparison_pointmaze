...
protocol and metric, one commit and one ledger row per experiment. Thirty-plus experiments later, the answer has three layers, developed in
\Cref{sec:iteration-experiments,sec:nonuniform,sec:capacity}: an exact construction that
solves the uniform regime to the achievable limit, a statistical construction that tracks
per-position counts under arbitrary visitation, and a precise account of \emph{why} ordinary
optimizers cannot do either.
...
residual-encoded shrink (exact solve) & \textbf{0.038} & 0.038 & 0.00 & -0.500 & 0.000 \\
coin flips, adaptive dictionary & \underline{0.054} & \underline{0.030} & 0.00 & -0.502 & 0.003 \\
elliptical posterior readout ($\sigma \le 0.35$) & 0.139 & 0.046 & 0.00 & -0.499 & 0.001 \\
Hadamard coins (column-permuted), adaptive dictionary & 0.314 & \textbf{0.008} & 0.00 & -0.501 & 0.001 \\
...
\item \textbf{The third point set breaks the optimizer family further.} AdaGrad's pooled slope
  ...
  ($0.0381$; $0.054$).
\item \textbf{The Hadamard refinement needed its own bug found.} Orthogonal coin blocks
  scrambled by signs alone send every full block's sum onto one coordinate, and consecutive
  blocks cancel: past $512$ visits the running-mean norm collapses (val 109--111, kept as the
  record of the broken variant). A per-block column permutation restores exactness at every
  horizon (val 114--116).
...
% (written after the bibliography verification pass; see literature/ for the full notes)
...
\begin{enumerate}[leftmargin=*]
\item \textbf{Off-support behavior.} The metric evaluates on the visited point set. The
  winning constructions generalize through their features (kernel dictionary, prior
  network), but how the bonus field behaves \emph{between} training positions --- the
  quantity an exploring agent actually queries --- is unmeasured here.
