...
# NEW id, and the id file is your ONLY legal scancel source; (2) snapshot your jobs + both queue
# pools; (3) auto-refill by re-running the launcher — it self-gates on queue depth and your caps, so
# calling it every cycle only ever ADDS jobs when useful; (4) exit when the sweep is complete.
...
[[ "$USER" == "sl5nw" ]] && { echo "you are the sweep owner — use the run's slurm/monitor.sh instead"; exit 3; }
...
while true; do
  ...
  # snapshot my jobs + both pools' depth
  squeue -u "$USER" -h -o "%.10i %.9j %.9T %.5C %.16R" | grep -E "$PREFIX_RE" || echo "(none of my worker jobs in squeue)"
  echo "[queue] pending_1m=$(ls "$POOL_1M" 2>/dev/null | wc -l) pending_10m=$(ls "$POOL_10M" 2>/dev/null | wc -l) running=$(ls "$RUN_DIR/queue/$SWEEP_ID/running" 2>/dev/null | wc -l)"
