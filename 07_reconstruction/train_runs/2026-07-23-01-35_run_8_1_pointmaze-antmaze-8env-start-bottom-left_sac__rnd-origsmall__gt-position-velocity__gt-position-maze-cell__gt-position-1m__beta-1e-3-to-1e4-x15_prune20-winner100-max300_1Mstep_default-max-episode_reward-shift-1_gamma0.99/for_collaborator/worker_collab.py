#!/usr/bin/env python
"""Collaborator worker for Run 8.1: the run's own claim-run-mark loop plus a fail-fast guard.

Reuses the run's own slurm/worker.py unchanged (its claim window, train.py argv builder incl.
--env_setup and the WORKER_DEVICE override, and its marker moves). The one addition: if the first
MAX_FAST_FAILURES claimed configs ALL fail after less than FAST_FAIL_SECONDS each (an import-time /
environment / permissions failure, not 9-25 h of real training), this worker STOPS claiming, writes
ONE problem report naming the markers it sent to failed/, and exits loudly. It NEVER renames anything
back itself — the collaborator side never repairs shared state; the owner's monitor loop
(slurm/requeue_orphans.py) reads the report and requeues the named markers (coordination contract).

Env (set by the *_collab.slurm scripts): RUN_DIR, PROJ_DIR, SWEEP_ID, WORKER_DEVICE=cpu.
"""
import getpass
import os
import sys
...
def main():
    ...
    while True:
        # claim one pending config atomically (rename into running/); None when the queue is empty
        ...
        if name is None:
            ...
            return 0
        # load the claimed config and run train.py with the run's own argv builder (env_setup + device)
        import json as _json
        with open(path) as fh:
            cfg = _json.load(fh)
        worker.log(f"claimed {name} :: {cfg.get('env_setup', '?')} {cfg['algorithm']} "
                   f"beta={cfg['beta']} seed={cfg['a_seed']} params={cfg.get('params', {})}")
        t0 = time.time()
        import subprocess
