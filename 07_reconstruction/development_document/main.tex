...
\end{table}
\subsection{Train run 3 (proposed): elliptical bonuses and RND optimizers}
...
\subsubsection{Part I: elliptical bonuses}
...
\(K_\Lambda\) & Cholesky refresh cadence & Number of covariance updates between two Cholesky refactorizations. \\
...
Bonus clip &
\(\{\mathbf{\infty},10\}\) &
The unclipped value tests the intended formula.  A finite clip is a stability
check if rare reward spikes destabilize SAC. &
try \\
\bottomrule
\end{tabular}%
}
\caption{Common numerical settings for all elliptical methods.  The main first-stage
run fixes \(d_\varphi=128\), unit-norm features, adaptive Cholesky jitter, and no
bonus clipping.}
...
Cholesky factorization costs \(O(d_\varphi^3)\), so the factor can be cached and
refreshed every \(K_\Lambda\) covariance updates.  The exact setting is
\(K_\Lambda=1\).

With unit-norm features, the theory-aligned ridge is
\[
\lambda=1.
\]
It acts like one prior visit in every feature direction and bounds the initial
bonus by \(1\).  This is the main reason the ridge sweep is reduced.
...
After unit normalization and \(\lambda=1\), the raw bonus is on approximately the
...
\(\lambda\) &
Ridge regularizer &
\(\{\mathbf{1},10^{-2},\textcolor{gray}{10^{-4}}\}\) &
\(\lambda=1\) is the count-prior value for unit-norm cumulative features.
\(\lambda=10^{-2}\) tests more aggressive exploration.  The gray value tests
near-singular behavior and should not be in the main grid. &
try \\
\midrule
\(K_\Lambda\) &
Cholesky refresh cadence &
\(\{\mathbf{1},16\}\) &
\(K_\Lambda=1\) is exact.  \(K_\Lambda=16\) tests whether cached factors reduce
CPU time without changing learning behavior. &
try \\
\bottomrule
\end{tabular}%
}
...
d_\varphi=256,\ r_{\max}^{\mathcal{E}}=10,\ \lambda=10^{-4}.}
\end{array}
\]
...
\(10^{-6}\) &
Diagonal prior in the per-batch covariance
\(\Lambda_B=\tfrac1n\Phi^\top\Phi+\lambda I\). \\
\midrule
--- &
...
\texttt{EllipticalBonus} class (the per-batch method E1 used by
\texttt{rnd\_elliptical}).  By default features are unit-norm normalized
...
%------------------------------------------------------------------------------
\subsubsection{Part II: RND optimizer variants}
