#!/usr/bin/env python3
"""
Scalar Ensemble Random Init-Based RND Method

Ensemble RND with:
- Same dataset for all K heads (no bootstrap sampling)
- Random initialization per head (sole diversity mechanism)
...
- Two uncertainty methods:
  - STD of errors: |target - prediction|
  - STD of predictions: prediction
- 10 total metrics: 5 metrics × 2 uncertainty types
"""
...
def parse_args():
    parser = argparse.ArgumentParser(description="Scalar Ensemble Random Init-Based RND Testing")
    ...
    parser.add_argument("--gaussian_noise", type=float, default=0.0, help="Gaussian noise level (optional)")
    ...
    parser.add_argument("--project_name", type=str, default="scalar-ensemble-random-init-rnd", help="WandB project name")
...
def run_single_experiment(args, device, run_idx):
    ...
    # Create scalar ensemble random init RND method
    ...
    # Train ensemble
    print(f"Training Scalar Ensemble Random Init RND with {args.num_heads} predictors...")
...
def main():
    ...
    if wandb.run is not None:
        ...
        print("🔄 Running in WandB sweep mode - Scalar Ensemble Random Init RND")
    else:
        if args.wandb_switch.lower() == "true":
            wandb.init(
                project=args.project_name,
                config=vars(args),
                name=f"scalar_ensemble_random_init_rnd_num_heads{args.num_heads}_noise{args.gaussian_noise}"
            )
    ...
    print(f"Starting Scalar Ensemble Random Init-Based RND testing")
    ...
    save_heatmap_to_wandb(
        ...
        title=f"Scalar Ensemble Random Init RND (Errors) (num_heads={args.num_heads}, gaussian_noise={args.gaussian_noise}) - Final Run",
        wandb_switch=(args.wandb_switch.lower() == "true")
    )
    
    save_heatmap_to_wandb(
        final_result['uncertainty_matrix_predictions'],
        title=f"Scalar Ensemble Random Init RND (Predictions) (num_heads={args.num_heads}, gaussian_noise={args.gaussian_noise}) - Final Run",
        wandb_switch=(args.wandb_switch.lower() == "true")
    )
