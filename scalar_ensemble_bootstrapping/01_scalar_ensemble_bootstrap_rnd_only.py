...
def run_single_experiment(args, device, run_idx):
    ...
    # Train ensemble
    print(f"Training Scalar Ensemble Bootstrap RND with K={args.K} predictors...")
...
def main():
    ...
    if wandb.run is not None:
        ...
        if args.wandb_switch.lower() == "true":
            wandb.init(
                ...
                config=vars(args),
                name=f"scalar_ensemble_bootstrap_rnd_K{args.K}_noise{args.gaussian_noise}"
            )
    ...
    save_heatmap_to_wandb(
        ...
        title=f"Scalar Ensemble Bootstrap RND (K={args.K}, gaussian_noise={args.gaussian_noise}) - Final Run",
        wandb_switch=(args.wandb_switch.lower() == "true")
    )
