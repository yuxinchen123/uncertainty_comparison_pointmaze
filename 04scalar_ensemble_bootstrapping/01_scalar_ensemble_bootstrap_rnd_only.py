#!/usr/bin/env python3
"""
Scalar Ensemble Bootstrap-Based RND Method
...
from scalar_ensemble_bootstrapping_methods import ScalarEnsembleBootstrapRNDMethod
...
def parse_args():
    parser = argparse.ArgumentParser(description="Scalar Ensemble Bootstrap-Based RND Testing")
    ...
    parser.add_argument("--project_name", type=str, default="scalar-ensemble-bootstrap-rnd", help="WandB project name")
...
def run_single_experiment(args, device, run_idx):
    ...
    # Create scalar ensemble bootstrap RND method
    ...
    # Train ensemble
    print(f"Training Scalar Ensemble Bootstrap RND with {args.num_heads} predictors...")
...
def main():
    ...
    if wandb.run is not None:
        ...
        print("🔄 Running in WandB sweep mode - Scalar Ensemble Bootstrap RND")
    else:
        if args.wandb_switch.lower() == "true":
            wandb.init(
                project=args.project_name,
                config=vars(args),
                name=f"scalar_ensemble_bootstrap_rnd_num_heads{args.num_heads}_noise{args.gaussian_noise}"
            )
    ...
    print(f"Starting Scalar Ensemble Bootstrap-Based RND testing")
    ...
    save_heatmap_to_wandb(
        ...
        title=f"Scalar Ensemble Bootstrap RND (Errors) (num_heads={args.num_heads}, gaussian_noise={args.gaussian_noise}) - Final Run",
        wandb_switch=(args.wandb_switch.lower() == "true")
    )
    
    save_heatmap_to_wandb(
        final_result['uncertainty_matrix_predictions'],
        title=f"Scalar Ensemble Bootstrap RND (Predictions) (num_heads={args.num_heads}, gaussian_noise={args.gaussian_noise}) - Final Run",
        wandb_switch=(args.wandb_switch.lower() == "true")
    )
